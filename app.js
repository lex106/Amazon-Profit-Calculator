...
function calculateProfit() {
  ...
  const monthlySales = Number($("#monthlySales").value) || 0;
  ...
  const referralFee = Math.max(salePrice * params.referralRate, params.minFee);
  const advertisingFee = salePrice * advertisingRate;
  const returnFee = state.fee ? state.fee.returnProcessingFee : 0;
  const storageFee = state.fee ? state.fee.storageFee : 0;
  const totalCost = productCostLocal + firstLegCost + fulfillmentFee + referralFee + advertisingFee + returnFee + storageFee;
  const profit = salePrice - totalCost;
  const margin = salePrice > 0 ? profit / salePrice : 0;
  const monthlyGross = profit * monthlySales;
  const annualGross = monthlyGross * 12;
  const shippedEnough = monthlySales >= RETURN_RULES.minimumMonthlyShipments;
  const threshold = category === "other" ? 0.048 : RETURN_RULES[category];
  const returnChargeApplies = shippedEnough && returnRate > threshold;
...
function renderProfit() {
  ...
  $("#monthlyProfit").textContent = fmtMoney(p.monthlyGross);
  $("#monthlyProfitCny").textContent = `≈ ${fmtCny(p.monthlyGross * p.exchangeRate)}`;
  $("#annualProfit").textContent = fmtMoney(p.annualGross);
  $("#annualProfitCny").textContent = `≈ ${fmtCny(p.annualGross * p.exchangeRate)}`;
  ...
  $("#returnRuleText").textContent = p.returnChargeApplies
    ? `当前月退货率 ${fmtPct(p.returnRate)}，高于阈值 ${fmtPct(p.threshold)}，会触发退货处理费。`
    : `当前月退货率 ${fmtPct(p.returnRate)}，阈值 ${fmtPct(p.threshold)}；若月发货少于 25 件则不计费。`;
}
...
function bindEvents() {
  ...
  $$("#feeForm input, #feeForm select").forEach((el) => el.addEventListener("input", calculateFee));
  $$("#profitForm input, #profitForm select").forEach((el) => el.addEventListener("input", renderProfit));
